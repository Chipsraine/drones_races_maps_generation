...
from environment_with_flags import GateEnvironmentWithFlags, WORK_MIN, WORK_MAX, ARENA_SIZE, MAX_GATES, MAX_FLAGS
...
def collect_episode(env: GateEnvironmentWithFlags, policy: PolicyNetwork, n_gates: int, n_flags: int = None) -> dict:
...
def main():
    ...
    task = Task.init(project_name="DroneTrack", task_name=f"RL v6.0 Flags - {env_name}")
    task.connect({
    ...
    policy = PolicyNetwork()
       ...
       # === ЗАГРУЗКА ВЕСОВ ИЗ 5 ВОРОТ ===
    checkpoint_path = SAVE_DIR / "best_model_5gates_96percent.pt"
    if checkpoint_path.exists():
        policy.load_state_dict(torch.load(checkpoint_path, weights_only=True))
        print(f"✅ Загружены веса из 5 ворот: {checkpoint_path}")
        print("   Дообучаем на 6 воротах...")
    else:
        print("⚠️ Веса не найдены, начинаем с нуля")
    ...
    for iteration in range(1, N_ITERATIONS + 1):
        ...
        for _ in range(EPISODES_PER_ITER):
            ...
            n_flags = np.random.randint(0, min(n_gates + 1, MAX_FLAGS + 1))  # Флагов от 0 до n_gates
    ...
    final = evaluate(env, policy, 500)  # только 5 ворот (исправлено)
