...
class GateEnvironment:
    ...
    def _evaluate_config(self) -> tuple[float, dict]:
        ...
        # 8a. Столбики в пределах арены
        for i in range(MAX_PILLARS):
            if WORK_MIN <= pillars[i, 0] <= WORK_MAX and WORK_MIN <= pillars[i, 1] <= WORK_MAX:
                reward += 0.3
            else:
                reward -= 1.0
                info["violations"].append(f"pillar_bounds(p{i})")

        # 8b. Путь не проходит через столбик
        pillar_hit = 0
        for pi in range(MAX_PILLARS):
            ...
            for i in range(n):
                ...
                if dist < PILLAR_RADIUS:
                    pillar_hit += 1
                    break  # один столбик считаем один раз

        if pillar_hit > 0:
            reward -= 2.0 * pillar_hit
            info["violations"].append(f"path_through_pillar({pillar_hit})")
        else:
            reward += 2.0
            info["bonuses"].append("pillars_avoided")

        # 8c. Столбики не слишком близко к воротам
        for pi in range(MAX_PILLARS):
            for gi in range(n):
                ...
                if d < PILLAR_MIN_DIST_GATE:
                    reward -= 1.0
                    info["violations"].append(f"pillar_near_gate(p{pi},g{gi})")
                    break

        # 8d. Столбики не слишком близко друг к другу
        if MAX_PILLARS >= 2:
            d_pp = np.hypot(pillars[0, 0] - pillars[1, 0],
                            pillars[0, 1] - pillars[1, 1])
            if d_pp < PILLAR_MIN_DIST_PILLAR:
                reward -= 1.0
                info["violations"].append(f"pillars_too_close({d_pp:.1f}m)")
            else:
                reward += 0.5

        # 8e. Бонус: столбик рядом с путём (1-3м) — делает трассу интереснее
        for pi in range(MAX_PILLARS):
            px, py = pillars[pi]
            ...
            for i in range(n):
                ...
                d = _point_to_segment_dist(px, py,
                                           config[i, 0], config[i, 1],
                                           config[j, 0], config[j, 1])
                min_seg_dist = min(min_seg_dist, d)
            if PILLAR_RADIUS < min_seg_dist <= 3.0:
                reward += 1.0
                info["bonuses"].append(f"pillar_near_path(p{pi},{min_seg_dist:.1f}m)")
