...
ACTION_DIM = MAX_GATES * 3 + MAX_PILLARS * 2  # 18 + 4 = 22
...
class GateEnvironment:
    ...
    def step(self, action: np.ndarray) -> tuple[np.ndarray, float, bool, dict]:
        ...
        # Декодируем столбики (после ворот в action)
        pillar_offset = MAX_GATES * 3
        pillars = []
        for i in range(MAX_PILLARS):
            px = action[pillar_offset + i * 2]     * WORK_RANGE + WORK_MIN
            py = action[pillar_offset + i * 2 + 1] * WORK_RANGE + WORK_MIN
            pillars.append((px, py))
        self.pillars = np.array(pillars, dtype=np.float32)
    ...
    def _evaluate_config(self) -> tuple[float, dict]:
        ...
        # === БАЗОВЫЙ БОНУС: просто за попытку ===
        reward += 2.0 * n  # +6 за 3 ворота, +12 за 6
        ...
        for i in range(n):
            if WORK_MIN <= config[i, 0] <= WORK_MAX and WORK_MIN <= config[i, 1] <= WORK_MAX:
                ...
                reward -= 0.5  # МЕНЬШЕ штраф!
                info["violations"].append(f"bounds(g{i})")

        # === 2. Дистанции между соседними (КЛЮЧЕВОЕ!) ===
        dist_ok = 0
        dist_scores = []
        for i in range(n):
            j = (i + 1) % n
            ...
            # НОВОЕ: плавная награда за близость к идеалу
            ideal = (MIN_DIST + MAX_DIST) / 2
            dist_score = max(0, 1.0 - abs(d - ideal) / (MAX_DIST - MIN_DIST))
            dist_scores.append(dist_score)
            
            if MIN_DIST <= d <= MAX_DIST:
                reward += 2.0 * dist_score  # до +2 за идеальную дистанцию
                dist_ok += 1
            elif d < MIN_DIST:
                reward -= 1.0 * (MIN_DIST / max(d, 0.1))  # плавный штраф
                info["violations"].append(f"too_close({i}→{j},{d:.1f}m)")
            else:
                reward -= 1.0 * (d / MAX_DIST)  # плавный штраф
                info["violations"].append(f"too_far({i}→{j},{d:.1f}m)")

        if dist_ok == n:
            reward += 10.0  # БОЛЬШОЙ бонус за все дистанции OK
        ...
        for i in range(n):
            ...
            if diff <= MAX_ANGLE_DIFF:
                reward += 1.0  # УВЕЛИЧЕНО с 0.5
            else:
                reward -= 0.5  # УМЕНЬШЕНО с 1.0
        ...
        for i in range(n):
            for j in range(i + 2, n):
                ...
                if d < GLOBAL_MIN_DIST:
                    reward -= 1.0  # УМЕНЬШЕНО с 2.0
                    global_close += 1
                    info["violations"].append(f"global_close({i},{j},{d:.1f}m)")

        # === 5. Пересечения сегментов (КРИТИЧНО!) ===
        crossing_count = 0
        for i in range(n):
        ...
        if crossing_count > 0:
            reward -= 3.0 * crossing_count  # УМЕРЕННО (было 2.0)
            info["violations"].append(f"crossings({crossing_count})")
        else:
            reward += 5.0  # УВЕЛИЧЕНО с 3.0
        ...
        if sharp_turns > 0:
            reward -= 0.5 * sharp_turns  # УМЕНЬШЕНО с 1.0
            info["violations"].append(f"sharp_turns({sharp_turns})")
        else:
            reward += 3.0  # УВЕЛИЧЕНО с 1.0
            info["bonuses"].append("smooth_path")

        # === 7. Coverage-бонус (УСИЛЕН) ===
        coords = config[:, :2]
        spread = coords.std(axis=0).mean()
        reward += spread * 3.0  # УВЕЛИЧЕНО с 1.0
        ...
        pillar_bounds_ok = 0
        for i in range(MAX_PILLARS):
            if WORK_MIN <= pillars[i, 0] <= WORK_MAX and WORK_MIN <= pillars[i, 1] <= WORK_MAX:
                reward += 0.5
                pillar_bounds_ok += 1
            else:
                reward -= 0.3

        # 8b. Путь не через столбик
        pillar_hit = 0
        for pi in range(MAX_PILLARS):
            px, py = pillars[pi]
            for i in range(n):
                j = (i + 1) % n
                dist = _point_to_segment_dist(px, py,
                                              config[i, 0], config[i, 1],
                                              config[j, 0], config[j, 1])
                if dist < PILLAR_RADIUS:
                    pillar_hit += 1
                    break

        if pillar_hit > 0:
            reward -= 1.0 * pillar_hit  # УМЕНЬШЕНО с 2.0
            info["violations"].append(f"path_through_pillar({pillar_hit})")
        else:
            reward += 3.0  # УВЕЛИЧЕНО с 2.0
            info["bonuses"].append("pillars_avoided")

        # 8c-e. УПРОЩЕНО: убраны сложные проверки столбиков
        # (оставляем только базовые, чтобы агент сфокусировался на воротах)

        # === ФИНАЛЬНЫЙ БОНУС ===
        n_violations = len(info["violations"])
        if n_violations == 0:
            reward += 20.0  # ОГРОМНЫЙ бонус за идеальную трассу!
            info["bonuses"].append("PERFECT_TRACK")
    ...
    def is_valid(self) -> bool:
        if self.config is None or self.pillars is None:
            return False
        ...
        # Столбики: путь не проходит через них
        for pi in range(MAX_PILLARS):
            px, py = pillars[pi]
            if not (WORK_MIN <= px <= WORK_MAX and WORK_MIN <= py <= WORK_MAX):
                return False
            for i in range(n):
                j = (i + 1) % n
                dist = _point_to_segment_dist(px, py,
        ...
        # Столбики не слишком близко к воротам
        for pi in range(MAX_PILLARS):
            for gi in range(n):
                d = np.hypot(pillars[pi, 0] - config[gi, 0],
                             pillars[pi, 1] - config[gi, 1])
                if d < PILLAR_MIN_DIST_GATE:
                    return False
