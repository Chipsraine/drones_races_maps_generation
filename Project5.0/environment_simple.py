...
class GateEnvironment:
    ...
    def step(self, action: np.ndarray) -> tuple[np.ndarray, float, bool, dict]:
        ...
        reward, info = self._evaluate_config()
                # Бонус за валидность при большом числе ворот
        #n_violations = len(info["violations"])
        #if n >= 5 and n_violations <= 3:
        #    reward += 20.0 * (5 - n_violations)  # до 60-80 очков!
        #    info["bonuses"].append(f"almost_valid_{n}_gates")
        
        #if n >= 5 and n_violations == 0:
        #    reward += 100.0  # КОСМИЧЕСКИЙ бонус за идеальные 5-6 ворот!
        #    info["bonuses"].append(f"PERFECT_{n}_GATES")
                # === ПЛОТНАЯ НАГРАДА ЗА "ПОЧТИ ВАЛИДНО" ===
        # Чем меньше нарушений — тем больше бонус
        n_violations = len(info["violations"])
        if n == 5:
            if n_violations == 0:
                reward += 50.0  # Идеально!
            elif n_violations == 1:
                reward += 30.0
            elif n_violations == 2:
                reward += 15.0
